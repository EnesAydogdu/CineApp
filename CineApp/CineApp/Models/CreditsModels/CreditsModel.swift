...
class CreditsModel: Codable {
    
    let cast: [CreditsServiceModel]
    let crew: [CreditsServiceModel]
    
    init(cast: [CreditsServiceModel], crew: [CreditsServiceModel]) {
        self.cast = cast
        self.crew = crew
    }
}

class CreditsServiceModel: Codable {
